...
<script setup>
  ...
  const estimateFileSize = async () => {
    ...
    try {
      ...
      // 计算采样尺寸（最大边不超过 200px，保证速度）
      let sampleWidth = resultWidth.value;
      let sampleHeight = resultHeight.value;
      const maxSampleSize = 200;

      if (sampleWidth > maxSampleSize || sampleHeight > maxSampleSize) {
        const ratio = Math.min(maxSampleSize / sampleWidth, maxSampleSize / sampleHeight);
        sampleWidth = Math.round(sampleWidth * ratio);
        sampleHeight = Math.round(sampleHeight * ratio);
      }
      ...
      if (outputFormat.value !== 'original') {
        switch (outputFormat.value) {
          case 'webp':
            mimeType = 'image/webp';
            qualityValue = quality.value / 100;
            break;
          case 'jpeg':
            mimeType = 'image/jpeg';
            qualityValue = quality.value / 100;
      ...
      // 采样压缩
      const sampleBlob = await new Promise((resolve) => {
        if (qualityValue !== undefined) {
          tempCanvas.toBlob(resolve, mimeType, qualityValue);
        } else {
      ...
      if (!sampleBlob) {
        throw new Error('采样失败');
      }

      // 根据面积比例推算完整大小
      const sampleArea = sampleWidth * sampleHeight;
      const fullArea = resultWidth.value * resultHeight.value;
      const areaRatio = fullArea / sampleArea;

      let correctionFactor = 1.0;
      if (fullArea > 2000000) {
        // 大于 200万像素（约 2000x1000）
        correctionFactor = 0.92; // 大图通常压缩更好
      } else if (fullArea < 50000) {
        // 小于 5万像素
        correctionFactor = 1.08; // 小图压缩率略差
      }

      estimatedSize.value = Math.round(sampleBlob.size * areaRatio * correctionFactor);
    } catch (err) {
      console.warn('采样预估失败，回退到公式估算:', err);
      ...
      const originalBytesPerPixel = originalSize.value / originalPixels;

      if (outputFormat.value === 'original') {
        // 按像素比例估算，但考虑图片复杂度
        let complexityFactor = 1.0;
        if (originalBytesPerPixel > 0.5) {
          complexityFactor = 0.85;
        } else if (originalBytesPerPixel < 0.2) {
          // 原图已高度压缩
          complexityFactor = 1.15;
        }
        estimatedBytes = originalSize.value * (totalPixels / originalPixels) * complexityFactor;
      } else {
        let bytesPerPixel;
        ...
        switch (outputFormat.value) {
          case 'webp':
            // WebP 压缩效率：高质量接近原图，低质量大幅减少
            bytesPerPixel = 0.15 + 0.35 * Math.pow(q, 1.2);
            break;
          case 'jpeg':
            // JPEG 经验公式（基于大量图片测试）
            // 质量 100%: ~0.5-0.8 字节/像素
            // 质量 70%: ~0.2-0.3 字节/像素
            // 质量 50%: ~0.1-0.15 字节/像素
            bytesPerPixel = 0.08 + 0.45 * Math.pow(q, 1.5);
            break;
          case 'png':
            // PNG 无损压缩，与图片复杂度高度相关
            // 简单估算：简单图形小，复杂照片大
            const complexity = Math.min(1.5, originalBytesPerPixel / 0.3);
            bytesPerPixel = 0.8 * complexity;
            break;
          default:
            bytesPerPixel = originalBytesPerPixel * 0.7;
        }

        // 添加质量下限保护
        bytesPerPixel = Math.max(0.05, Math.min(1.2, bytesPerPixel));
        estimatedBytes = totalPixels * bytesPerPixel;
      ...
      estimatedSize.value = Math.max(Math.round(estimatedBytes), 100);
    }
  };
